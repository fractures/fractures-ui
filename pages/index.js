	...
	render() {
		return (
			<Layout>
				<Box title={ null }>
					<div className="py-6">
						<Helios colors={ this.state.theme.colors } />
				...
				<Box title="Color" component={ Color }>
					<div className="grid grid-cols-3 grid-gap-3">
						{Object.entries(this.state.theme.colors).map((color, key) => {
							...
							return (
								<div className="flex flex-column" key={ key }>
									{name.map((colorMap, colorKey) => {
										...
										const background = this.state.theme.colors.gray[0]
				...
				<Box title="Button" component={ Button }>
					<Line isRow>
						<Button type="primary" value="Default" onClick={ this.toggleLoading } />
					...
					<Line isRow>
						<Button type="primary" value="Activate" isLoading={ !this.state.isLoading } onClick={ setValue } />
					...
					<Line isRow>
						<Button type="secondary" value="Activate" isLoading={ !this.state.isLoading } onClick={ setValue } />
						<Button type="secondary" value="Activate" isRounded={ true } isLoading={ false } onClick={ setValue } />
						<Button type="secondary" value="Activate" isRounded={ true } onClick={ setValue } isLoading={ !this.state.isLoading } />
					</Line>
				</Box>
				...
				<Box title="Type">
					...
					<p>
						...
						By the same illusion which lifts the horizon of the sea to the level of the spectator on a hillside, the sable cloud beneath was
						dished out, and the car seemed to float in the middle of an immense dark sphere, whose upper half was strewn with silver.
